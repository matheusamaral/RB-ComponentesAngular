...
class ConsultaMapaFiltro {
    ...
    public function consultar($usuarioId, $latitude, $longitude, $tempoHashtag, $tempoMidia, 
            $categoriaLocal, $categoriaHashtag){
        ...
        $query->join('categoria_hashtag', 'ch', 'left')->on('ch.id = hc.categoria_hashtag_id')
                ->on('ch.ativo = 1');
        $query->join('local_categoria', 'lc', 'left')->on('lc.local_id = l.id')
                ->on('lc.ativo = 1');
        $query->join('categoria_local', 'cl', 'left')->on('cl.id = lc.categoria_id')
                ->on('cl.ativo = 1');
        $query->where('l.ativo = 1')
                ->add('(ch.id in(?) or cl.id in(?))');
        $query->group('l.id');
        ...
        $query->addVariaveis([$latitude, $longitude, $latitude, 
            ...
            $categoriaHashtag, $categoriaLocal]);
